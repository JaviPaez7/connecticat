...
export default function HeroLanding() {
  ...
  return (
    <section className="relative isolate min-h-[calc(100svh-4.5rem)] w-full overflow-hidden">
      ...
      >
        <img
          ...
          fetchPriority="high"
        />
        <div className="absolute inset-0 bg-gradient-to-r from-ink-900/80 via-ink-900/55 to-coral-700/25" />
      ...
      <motion.div
        ...
        animate={reduce ? undefined : { y: [0, -18, 0], opacity: [0.35, 0.55, 0.35] }}
        transition={{ duration: 7, repeat: Infinity, ease: 'easeInOut' }}
      />
      <motion.div
        aria-hidden
        className="pointer-events-none absolute -right-16 bottom-24 h-80 w-80 rounded-full bg-mint-300/20 blur-3xl"
        animate={reduce ? undefined : { y: [0, 16, 0], opacity: [0.25, 0.45, 0.25] }}
        transition={{ duration: 8.5, repeat: Infinity, ease: 'easeInOut', delay: 0.4 }}
      />

      <div className="relative mx-auto flex min-h-[calc(100svh-4.5rem)] max-w-6xl flex-col justify-center px-4 py-16 sm:px-6">
        <motion.p
        ...
        >
          ...
          <motion.a
            ...
            whileHover={reduce ? undefined : { scale: 1.04, y: -2, backgroundColor: 'rgba(255,255,255,0.18)' }}
            whileTap={reduce ? undefined : { scale: 0.98 }}
          >
            Probar Gatinder
          </motion.a>
